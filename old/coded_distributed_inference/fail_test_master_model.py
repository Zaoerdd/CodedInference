...
def test_distributed_coded_convolution_k(master_ip: str, CNN_model: nn.Module, repetition):
    ...
    stop_thread = True
    recv_thread.join()
    fail_test = True  # 手动fail or 概率fail
    fail_num = 0
    ...
    layers_to_test = [0, 1, 3, 4, 6, 7, 8, 10, 11, 12, 14, 15, 16]
    ...
    for layer_num in layers_to_test:
        ...
        records = []
        for k in [9]:  # test k from 2 to n-1, and extract k = 5 and optimal k*
    ...
    if fail_test:
        record_file = 'record/'+f'vgg16_r{repetition}_t1_fail{fail_num}.rec'
    else:
        record_file = 'record/'+f'vgg16_r{repetition}_t1.rec'

    with open(record_file, 'wb') as f:
        pickle.dump(records_layers, f)
...
def test_distributed_repetitive_convolution_k(master_ip: str, CNN_model: nn.Module, repetition):
    ...
    for layer_num in layers_to_test:
        ...
        for r in tqdm(range(repetition)):
            ...
            if fail_test:  # send task (layer_num and k datas) to n workers
                fail_tasks = random.choice(range(n), fail_num, replace=False)
        ...
        record_layers.append(records)
    if fail_test:
        record_file = 'record/'+f'vgg16_r{repetition}_t2_fail{fail_num}.rec'
    else:
        record_file = 'record/'+f'vgg16_r{repetition}_t2.rec'
    with open(record_file, 'wb') as f:
        pickle.dump(record_layers, f)
    thread_is_working = False
    ...
    for _ in range(fail_num):
        ...
        for i, worker_socket in enumerate(worker_sockets):
            if i == target_idx:
                ...
                if not sent[i]:
                    send_data(worker_sockets[i], (0,0,0,xs[i]))
            sent[i] = True
...
def test_distributed_repetitive_convolution_N(master_ip: str, CNN_model: nn.Module, repetition):
    ...
    for layer_num in layers_to_test:
        ...
        for r in tqdm(range(repetition)):
            recv_list.clear()
            if fail_test and fail_num > 0:  # 先手动fail
                fail_set = random.choice(range(n), fail_num, replace=False)  # 随机选一个fail
                # print('Fail', fail_set)
            ...
            # receive coded outputs from k workers synchronously
            while not fail_list.empty() or len(recv_list) < n:
                time.sleep(1e-3)
        ...
        record_layer.append(records)

    if fail_test:
        record_file = 'record/'+f'vgg16_r{repetition}_t3_fail{fail_num}.rec'
    else:
        record_file = 'record/'+f'vgg16_r{repetition}_t3.rec'

    with open(record_file, 'wb') as f:
        pickle.dump(record_layer, f)
        print('Records saved')
