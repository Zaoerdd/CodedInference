...
def generate_layerConfigs(layers: list):
    ...
    for layer in layers:
        if isinstance(layer, BasicConv2d):
            ...
            layer_config = {'type': 'linear', 'in_features': layer.in_features,
                            'out_features': layer.out_features, 'bias': layer.bias is not None}
        elif isinstance(layer, nn.Dropout):
            layer_config = {'type': 'dropout', 'p': layer.p, 'inplace': layer.inplace}
        elif isinstance(layer, nn.Flatten):
            layer_config = {'type': 'flatten', 'start_dim': layer.start_dim, 'end_dim': layer.end_dim}
